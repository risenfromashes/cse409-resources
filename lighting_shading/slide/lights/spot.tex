    ...
    \begin{column}{0.3\textwidth}
      \begin{tikzpicture}[scale=0.8]
        ...
        \begin{scope}[shift={(3,3)}]
          ...
          \draw[->, blue, thick] (light) -- ($(light)+0.5*(surface)-0.5*(light)$) node[midway, above, anchor=south, black] {\footnotesize $\hat{\mathbf{L}}$};
  ...
  \begin{mathbox}{Complete Spot Light Formula}
    ...
    \only<4->{
      ...
      \begin{align*}
        I_{\text{final}} =
        \begin{cases}
          I_{\text{light}} \cdot (\cos(\alpha))^e \cdot \frac{1}{\epsilon + d^2} & \text{if } \cos(\alpha) > \cos(\theta_{\text{cutoff}}) \\
          0 & \text{otherwise}
        \end{cases}
      \end{align*}
    }
