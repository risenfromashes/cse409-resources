  ...
  \begin{columns}
    \begin{column}{0.7\textwidth}
      \begin{raybox}{Framebuffer Output}
        ...
        \textbf{Output:} Pixels written to display
  ...
  \begin{columns}
    ...
    \begin{column}{0.3\textwidth}
      \begin{tikzpicture}[scale=0.8]
        % Title
        \node at (1.25,4) {\textbf{Framebuffer Layout}};

        % Color Buffer
        \draw[thick, PrimaryColor] (0,3.2) rectangle (2.5,3.7);
        \node[left] at (-0.2,3.45) {\scriptsize Color};
        \foreach \x in {0.1,0.3,...,2.3} {
          \foreach \y in {3.3,3.5} {
            \fill[PrimaryColor!60] (\x,\y) rectangle ++(0.15,0.15);
          }
        }

        % Depth Buffer
        \draw[thick, SecondaryColor] (0,2.4) rectangle (2.5,2.9);
        \node[left] at (-0.2,2.65) {\scriptsize Depth};
        \foreach \x in {0.1,0.3,...,2.3} {
          \foreach \y in {2.5,2.7} {
            \pgfmathsetmacro\val{rnd*40 + 30}
            \fill[SecondaryColor!\val] (\x,\y) rectangle ++(0.15,0.15);
          }
        }

        % Stencil Buffer
        \draw[thick, AccentColor] (0,1.6) rectangle (2.5,2.1);
        \node[left] at (-0.2,1.85) {\scriptsize Stencil};
        \foreach \x in {0.1,0.3,...,2.3} {
          \foreach \y in {1.7,1.9} {
            \fill[AccentColor!40] (\x,\y) rectangle ++(0.15,0.15);
          }
        }

        % Pixel coordinate indicator
        \node[below] at (1.25,1.3) {\scriptsize Pixel (x, y)};
        \draw[dashed] (1.25,1.6) -- (1.25,3.7);
      \end{tikzpicture}
...
\begin{frame}{VSync \& Refresh Rates}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[below] at (3,-0.5) {\scriptsize Display Refresh (60 Hz)};

      % GPU frame completion
      \draw[thick, PrimaryColor] (0,-1) -- (6,-1);
      \fill[PrimaryColor] (0.8,-1) circle (2pt);
      \fill[PrimaryColor] (1.6,-1) circle (2pt);
      \fill[PrimaryColor] (2.9,-1) circle (2pt);
      \fill[PrimaryColor] (4.1,-1) circle (2pt);
      \fill[PrimaryColor] (5.2,-1) circle (2pt);
      \node[below] at (3,-1.2) {\scriptsize GPU Frame Completion};
