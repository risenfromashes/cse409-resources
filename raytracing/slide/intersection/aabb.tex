  ...
  \begin{columns}
    ...
    \begin{column}{0.35\textwidth}
      ...
      \begin{tikzpicture}[->, scale=0.35]
        ...
        \only<5>{
          \draw[ray] (3, -6) -- (3, 5);
          \draw[PrimaryColor, fill=PrimaryColor] (3, -6) circle (2pt) node[below] {$\mathbf{R}_o$};
          \draw[ray, red] (3, -6) -- (3, -4) node[midway, left] {$\mathbf{R}_d$};
        }
      \end{tikzpicture}
      \vspace{0.5cm}
      ...
      \only<5>{
        \scriptsize
        \raggedright
        Note that there is a special edge case when $x_{\min/\max} = R_{ox}$.
        To avoid this we can use a small epsilon value $\epsilon$:
        \begin{align*}
          t_{\min/\max,x} &= \frac{x_{\min/\max} - R_{ox} \mp \epsilon}{R_{dx}}
        \end{align*}
      }
    \end{column}
  \end{columns}
