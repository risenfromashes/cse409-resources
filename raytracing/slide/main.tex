...
\section{Ray Casting: The Foundation}

\begin{frame}{The Brilliant Insight}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{raybox}{Reverse Engineering Vision}
                Instead of following light from sources...\\
                \vspace{0.3cm}
                \highlight{Let's trace backwards from our eyes!}
            \end{raybox}
            
            \vspace{0.5cm}
            \textbf{Why does this work?}
            \begin{itemize}
                \item Most light never reaches our eyes
                \item Only trace rays that matter
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.6]
                ...
                \begin{scope}[canvas is zy plane at x=3.5]
                    ...
                    \draw[RayColor, very thick] (eye.center) -- (-1.5, -1.5);
                    \draw[->, RayColor, very thick] (-1.5, -1.5) -- (obj);
                    ...
                    \foreach \x in {-2.5,-1.5,...,2.5} {
                        \foreach \z in {-2.5,-1.5,...,2.5} {
                             \fill[red!70] (\x, \z) circle (0.08);
                             \draw[ray, thick, opacity=0.4] (eye.center) -- (\x,\z);
                        }
                    }
                \end{scope}
...
\begin{frame}{Ray Casting Algorithm}
    \begin{center}
        \begin{tikzpicture}[node distance=1.5cm]
            \node[process] (start) {For each pixel};
            \node[process, below of=start] (ray) {Shoot ray from eye};
            \node[process, below of=ray] (intersect) {Find closest intersection};
            \node[process, below of=intersect] (shade) {Calculate color};
            \node[process, below of=shade] (end) {Set pixel color};
            
            \draw[arrow] (start) -- (ray);
            \draw[arrow] (ray) -- (intersect);
            \draw[arrow] (intersect) -- (shade);
            \draw[arrow] (shade) -- (end);
            
            % Side illustration
            \begin{scope}[xshift=6cm, yshift=-1cm]
                \draw[step=0.3cm, gray, very thin] (0,0) grid (2,1.5);
                \node[eye] (eye) at (3,0.75) {};
                \node[sphere] (sphere) at (-1,0.75) {};
                \draw[ray] (eye) -- (0.6,0.75) -- (sphere);
                \node[pixel] at (0.6,0.75) {};
                \node[below] at (0.6,0.4) {\tiny Pixel};
            \end{scope}
        \end{tikzpicture}
    \end{center}
\end{frame}
